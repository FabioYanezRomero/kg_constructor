...
The `src/clients` module provides an abstraction layer over Large Language Model (LLM) backends. It provides a unified interface to execute knowledge graph extraction and general structural generation across different providers, such as Gemini, Ollama, and LM Studio.
...
*(Provider implementations are located in the `src/clients/providers/` directory.)*
...
1. Create a new module in `src/clients/providers/` (e.g., `openai.py`).
...
3. Register the new client in `src/clients/__init__.py`:
   ```python
...
The clients module is tightly integrated with the main Typer CLI (`src/__main__.py`), ensuring a seamless user experience.
