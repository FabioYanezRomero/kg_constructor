...
def run(
    ...
    prompt_path: Path = typer.Option(
        Path("src/prompts/legal_background_prompt.txt"),
        "--prompt-path",
...
def legal(
    csv_path: Path = typer.Option(
        Path("data/legal/sample_data.csv"),
        "--csv-path",
        help="Path to the legal CSV file with background summaries.",
    ),
    sample_size: Optional[int] = typer.Option(
        None,
        "--sample-size",
    ...
    prompt_path: Path = typer.Option(
        ...
        "--prompt-path",
        help="Prompt template to use when querying the model.",
    ),
    output_dir: Path = typer.Option(
        Path("data/output"),
        "--output-dir",
        "-o",
        help="Directory where JSON graph files will be written.",
    ),
    model: str = typer.Option(
        "abhishekchohan/gemma-3-12b-it-quantized-W4A16",
        "--model",
    ...
    api_key: Optional[str] = typer.Option(
        ...
        "--api-key",
        envvar="VLLM_API_KEY",
        help="Optional API key forwarded to vLLM.",
    ),
    request_timeout: int = typer.Option(
        120,
        "--timeout",
    ...
    """Generate knowledge graphs for legal backgrounds from the local CSV."""

    resolved_csv = csv_path.expanduser().resolve()
    os.environ.setdefault("HF_DATASETS_TRUST_REMOTE_CODE", "1")
    os.environ["LEGAL_BACKGROUND_SOURCE"] = str(resolved_csv)

    try:
        from datasets import config as hf_config  # type: ignore

        hf_config.HF_DATASETS_TRUST_REMOTE_CODE = True
    except Exception:  # pragma: no cover - defensive fallback if datasets missing
        pass

    dataset_config_obj = DatasetConfig.from_cli(
        dataset_name="src/kg_constructor/datasets/legal_background.py",
        dataset_config=None,
        splits=["train"],
        sample_size=sample_size,
    )
    ...
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(name)s - %(message)s")
    logging.info("Starting legal knowledge graph construction from %s", resolved_csv)
    build_and_save_graphs(run_config)
